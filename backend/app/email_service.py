...
async def send_activation_email(email: str, activation_token: str, user_name: str) -> bool:
    ...
    try:
        ...
        import os
        # Check environment variable first, then settings, then default to localhost
        base_url = os.getenv("FRONTEND_URL") or getattr(settings, 'frontend_url', None) or "http://localhost:3000"
